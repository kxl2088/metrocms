...
if ( ! file_exists('system/cms/config/database.php'))
{
	...
	{
		header('Status: 404');
		exit('MetroCMS is missing system/cms/config/database.php and cannot find the installer folder. Does your server have permission to access these files?');
	}
...
define('METRO_DEVELOPMENT', 'development');
define('METRO_STAGING', 'staging');
define('METRO_PRODUCTION', 'production');

define('ENVIRONMENT', (isset($_SERVER['METRO_ENV']) ? $_SERVER['METRO_ENV'] : METRO_DEVELOPMENT));
 ...
 */
	...
	switch (ENVIRONMENT)
	{
		case METRO_DEVELOPMENT:
			ini_set('display_errors', true);
		break;

		case METRO_STAGING:
		case METRO_PRODUCTION:
...
*/
	...
	if (ini_get('date.timezone') != 'America/Sao_Paulo')
	{
 ...
 */		
	...
	define('SHARED_ADDONPATH', 'apps/shared_apps/');
 ...
 *---------------------------------------------------------------
 *
 * Should MetroCMS run as a demo, meaning no destructive actions
 * can be taken such as removing admins or changing passwords?
 *
 */

    define('METRO_DEMO', (file_exists(FCPATH.'DEMO')));
