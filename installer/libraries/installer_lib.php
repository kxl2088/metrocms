 ...
 * @package MetroCMS\Installer\Libraries
 */
class Installer_lib
{
	...
	/**
	 * Install the MetroCMS database and write the database.php file
	 *
	...
	public function install($data)
	{
		...
		if ( ! $this->write_config_file())
		{
			return array('status' => false, 'message' => '', 'code' => 106);
                ...
                // Write the config file.
		if ( ! $this->write_rewrite_file())
		{
			return array('status' => false, 'message' => '', 'code' => 107);
		}
	...
	function write_config_file()
	{
		...
		$index_page = ($supported_servers[$server_name]['rewrite_support'] !== false) ? '' : 'index.php';
                $uri_protocol = ($server_name == 'nginx_w' || $server_name == 'nginx_wo') ? 'REQUEST_URI' : 'AUTO';
                
		return $this->_write_file_vars('../system/cms/config/config.php', './assets/config/config.php', array('__INDEX__' => $index_page, '__URI_PROTOCOL__' => $uri_protocol));
	}
        
        /**
	 * Writes the .htaccess or web.config
	 *
	 * @return bool
	 */
	function write_rewrite_file()
	{
		$server_name = $this->ci->session->userdata('http_server');

		// Able to use clean URLs?
                switch ($server_name){
                    case 'apache_w': $rewrite_file = ".htaccess"; break;
                    case 'iis_w': $rewrite_file = "web.config"; break;
                    default: $rewrite_file = false;
                }

		return ($rewrite_file ? $this->_write_file_vars('../' . $rewrite_file, './assets/config/' . $rewrite_file, array()) : true);
	}
