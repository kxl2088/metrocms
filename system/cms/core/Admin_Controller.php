...
/**
 * This is the basis for the Admin class that is used throughout MetroCMS.
 * 
...
class Admin_Controller extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->load->helper(array('admin_theme', 'avatar'));
		...
		ci()->theme_options = $this->metrocache->model('theme_m', 'get_values_by', array(array('theme' => ADMIN_THEME)));
		...
		if (is_logged_in())
		{
			...
			{				
				...
				if (method_exists($module['module'], 'admin_menu'))
				{
					$module['module']->admin_menu($menu_items);
                                ...
                                // If a module has an admin_add_menu function, then
				// we simply run that and allow it to manipulate the
				// menu array.
                                if (method_exists($module['module'], 'admin_add_menu'))
				{
					$module['module']->admin_add_menu($extra_menu);    
                                        foreach ($menu_items as $menu_key => &$menu_value){
                                            foreach ($extra_menu as $extra_key => $extra_value){
                                                if($extra_key == $menu_key){
                                                    $menu_value = $menu_value+(is_array($extra_value) ? $extra_value : array());
                                                }
                                            }
                                        }
				}
			...
			// edit profile links and such.
                        ...
                        $menu_items[$about] = array(
                                    'lang:global:site-view'			=> 'site-url'
                            );
                        
                        if( $this->current_user->group == 'admin' )
                        {                            
                            $menu_items[$about] += array(
                                    'lang:cp:nav_phpinfo'			=> 'admin/phpinfo'
                            );
                            $menu_items[$about] += array(
                                    'lang:cp:nav_info'                          => 'admin/info'
                            );
                        }
		...
		// ------------------------------
                ...
                // Get cPanel Api Status
                $cpanel_stats = new stdClass();
                $data = new stdClass();
                
                 $this->config->load('cpanel');    
                 
                if($this->config->item('cpanel_api_host') && $this->config->item('cpanel_api_port') && $this->config->item('cpanel_api_username') && $this->config->item('cpanel_api_password'))
                {
                    $this->load->library('cpanel_api');
                    // Get Account Status
                    $stats = $this->cpanel_api->stats_bar()->stats('diskusage|bandwidthusage');
                }
                else
                {
                    $stats = false;
                ...
                if( is_array($stats) )
                {
                    ...
                    {
                        $cpanel_stats->{$value->id} = $value;
                    }
                } 
                else if(is_object($stats) && !$stats->result)
                {                    
                    $this->template->set('messages', array('error' => sprintf(lang('cp:cpanel.error'), $stats->reason)));
                    $stats = false;
                }
                else
                {
                ...
                $data->cpanel_stats = ($stats && $cpanel_stats) ? $cpanel_stats : false;
                
		// Template configuration
		$this->template
			->enable_parser(false)
                        ->set('cpanel_stats', $data->cpanel_stats)
