...
class MY_Controller extends MX_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->library('metrocache');
                
		// Add the site specific theme folder
		$this->template->add_theme_location(ADDONPATH.'themes/');

		// Migration logic helps to make sure MetroCMS is running the latest changes
		...
		{
			log_message('debug', 'MetroCMS was migrated to version: ' . $schema_version);
		}
		...
		if ( ! ($admin_controller instanceof $this) and ($site_lang = AUTO_LANGUAGE))
		{
		...
		$metro['lang'] = $langs[CURRENT_LANGUAGE];
		$metro['lang']['code'] = CURRENT_LANGUAGE;

		$this->load->vars($metro);
