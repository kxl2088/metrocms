 ...
 * 
 * @author  	Parse19
 * @package  	MetroCMS\Core\Libraries\Streams\Drivers
 */ 
...
class Streams_utilities extends CI_Driver {
	 ...
	 * @return	bool
	 */
	public function remove_slug($slug, $namespace)
	{
		// Some field destructs use stream data from the cache,
		// so let's make sure that the slug cache has run.
		$this->CI->streams_m->run_slug_cache();
		...
		foreach ($streams as $stream)
		{
                        if($stream->stream_slug == $slug && $stream->stream_namespace == $namespace){
                            
                            $this->CI->streams_m->delete_stream($stream);
                            $this->CI->db->where('stream_id', $stream->id)->delete(ASSIGN_TABLE);
                        }
		}                
	}
	 ...
	 * @return	bool
	 */
	public function convert_column_to_field($stream_slug, $namespace, $field_name, $field_slug, $field_type, $extra = array(), $assign_data = array(), $locked = false)
	{
		...
		if ( ! $this->CI->fields_m->insert_field($field_name, $field_slug, $field_type, $namespace, $extra, $locked)) return false;
