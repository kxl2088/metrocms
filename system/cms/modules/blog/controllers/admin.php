...
class Admin extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('blog_m', 'blog_categories_m', 'blog_galleries_m'));
	...
	public function create()
	{
		...
		{
			$created_on = strtotime($this->input->post('created_on'));
		}
		else
		{
			$created_on = now();
		...
		if ($this->form_validation->run())
		{
			...
			{               
				...
				$this->metrocache->delete_all('blog_m');
		...
		else
		{
			...
			$post->type or $post->type = 'wysiwyg-advanced';
                ...
                // restore images
                $images = array();
                $images_post = $this->input->post('images');
                if($images_post)
                {
                    foreach ($images_post as $img)
                    {                        
		...
		$this->template
			...
			->append_js('jquery/jquery.tagsinput.js')
                        ->append_js('module::jquery.uploadify-3.2.min.js')
			->append_js('module::blog_form.js')
                        ->append_css('module::uploadify.css')
                        ->append_metadata($this->load->view('fragments/wysiwyg', array(), true))			
                        ->append_metadata("\n<script type=\"text/javascript\">\n\tvar uploadSWF = '".Asset::get_filepath_img('module::uploadify.swf', true)."';\n\tvar uploadURL = '".site_url('admin/blog/uploader/upload/'.($this->method == 'edit' ? $id : ''))."';\n\tvar uploadFolder = \"3\";\n\tvar uploadButtonText = '".lang('blog:select_files')."';\n\tvar uploadButtonStart = '".lang('blog:start_upload')."';\n\tvar uploadButtonStop = '".lang('blog:stop_upload')."';\n\tvar uploadButtonClear = '".lang('blog:cancel_upload')."';\n\tvar uploadButtonRemove = '".lang('global:remove')."';\n\tvar uploadFileTypeDesc = '".lang('blog:image_files')."';\n\tvar uploadAllowedExtensions = '*.gif; *.jpg; *.jpeg; *.png';\n\tvar uploadSizeLimit = '".Settings::get('files_upload_limit')."MB';\n\tvar uploadCookieName = '".$this->config->item('sess_cookie_name')."';\n\tvar uploadSessionId = '".$this->session->userdata('session_id')."';\n\tvar uploadSuccessMessage = '".lang('blog:image_success')."';\n\tvar uploadStopMessage = '".lang('blog:upload_stop')."';\n\tvar uploadClearMessage = '".lang('blog:upload_clear')."';\n</script>\n")
			->set('post', $post)
	...
	public function edit($id = 0)
	{
		...
		// Get the validation for our custom blog fields.
		$blog_validation = $this->streams->streams->validation_array($stream->stream_slug, $stream->stream_namespace, 'edit', array(), $id);
		...
		if ($this->form_validation->run())
		{
			...
			$extra = array(
				'title'            => $this->input->post('title'),
				'slug'             => $this->slug->create_uri(array('title' => $this->input->post('title')), $id),
				'category_id'      => $this->input->post('category_id'),
			...
			if ($this->streams->entries->update_entry($id, $_POST, $stream->stream_slug, $stream->stream_namespace, array('updated'), $extra))
			{
		...
		$post->created_on = $created_on;
                ...
                $images = $this->blog_galleries_m->get_many_by(array('blog_id' => $id));
		...
		if ( ! empty($ids))
		{
			...
			foreach ($ids as $id)
			{
				...
				if ($post = $this->blog_m->get($id))
				{
					...
					{
                                                $this->blog_galleries_m->delete_many_by(array('id' => $id));
        ...
        public function ajax_update_order()
	{
		...
		{
                        $order[$id] = $i;
                        $this->blog_galleries_m->update_order($id, array('sort' => $i));
			$i++;
		}
