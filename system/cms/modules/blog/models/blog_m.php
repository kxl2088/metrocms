...
class Blog_m extends MY_Model
{
	...
	public function get_tagged_by($tag, $params, $pagination)
	{
                if ( array_key_exists('limit', $pagination) and array_key_exists('offset', $pagination) )
                {
                    $this->db->limit($pagination['limit'], $pagination['offset']);
                }      
                
                // Count
                $total = $this->count_tagged_by($tag, $params);
                
                // Array result
		$result = array('entries' => $this->db->select('blog.*, blog.title title, blog.slug slug, blog_categories.title category_title, blog_categories.slug category_slug, profiles.display_name')
			->from('blog')
			->join('keywords_applied', 'keywords_applied.hash = blog.keywords')
			->join('keywords', 'keywords.id = keywords_applied.keyword_id')
