 ...
 *
 * @package		MetroCMS\Core\Modules\Comments\Controllers
...
class Comments extends Public_Controller
{
	...
	public function create($module = null)
	{
		...
		$comment = array(
			...
			'uri' 			=> $entry['uri'],
			'comment' 		=> Settings::get('comment_markdown') == 0 ? strip_tags($this->input->post('comment')) : $this->input->post('comment'),
		...
		if ($this->current_user)
		{
			...
			$comment['user_name'] = utf8_to_html($this->current_user->display_name);
	...
	private function _allow_comment()
	{
		...
		if (Settings::get('akismet_api_key'))
		{
			...
			{
				return array('status' => false, 'message' => lang('comments:spam_error'));
			}
		...
		{
			return array('status' => false, 'message' => lang('comments:added_on_blacklist'));
		}
