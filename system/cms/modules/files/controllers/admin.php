...
/**
 * MetroCMS file Admin Controller
 *
 * Provides an admin for the file module.
 *
 * @author		Jerel Unruh - MetroCMS Dev Team
 * @package		MetroCMS\Core\Modules\Files\Controllers
 */
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->template->append_metadata(
			"<script>                                
				metro.lang.fetching = '".lang('files:fetching')."';
				metro.lang.fetch_completed = '".lang('files:fetch_completed')."';
				metro.lang.start = '".lang('files:start')."';
				metro.lang.width = '".lang('files:width')."';
				metro.lang.height = '".lang('files:height')."';
				metro.lang.ratio = '".lang('files:ratio')."';
                                metro.lang.yes = '".lang('global:yes')."';
                                metro.lang.no = '".lang('global:no')."';
				metro.lang.full_size = '".lang('files:full_size')."';                                
                                metro.lang.watermark = \"".lang('files:watermark')."\";
				metro.lang.cancel = '".lang('buttons:cancel')."';
				metro.lang.synchronization_started = '".lang('files:synchronization_started')."';
				metro.lang.untitled_folder = '".lang('files:untitled_folder')."';
				metro.lang.exceeds_server_setting = '".lang('files:exceeds_server_setting')."';
				metro.lang.exceeds_allowed = '".lang('files:exceeds_allowed')."';
				metro.files = { permissions : ".json_encode(Files::allowed_actions())." };                                
				metro.files.max_size_possible = '".Files::$max_size_possible."';
				metro.files.max_size_allowed = '".Files::$max_size_allowed."';
				metro.files.valid_extensions = '".implode('|', $allowed_extensions)."';
				metro.lang.file_type_not_allowed = '".addslashes(lang('files:file_type_not_allowed'))."';
				metro.lang.new_folder_name = '".addslashes(lang('files:new_folder_name'))."';
				metro.lang.alt_attribute = '".addslashes(lang('files:alt_attribute'))."';
                                metro.lang.force_watermark = '".Settings::get('files_wm_force_use')."';

				// deprecated
				metro.files.initial_folder_contents = ".(int)$this->session->flashdata('initial_folder_contents').";
			</script>");
	}
	...
	public function rename_folder()
	{
		...
		if ($id = $this->input->post('folder_id') and $name = $this->input->post('name'))
		{
			...
			echo json_encode($result);
                    ...
                    {
                        echo json_encode(array('status' => false, 'message' => lang('files:no_permissions')));
                    }
		}
	}
	...
	public function upload()
	{
		...
		$input = $this->input->post();
                ...
                $input['ratio'] == 'yes' ? $ratio = true : $ratio = false;
                $input['watermark'] == 'yes' ? $wm = true : $wm = false;
                
		if($input['replace_id'] > 0)
		{
			$result = Files::replace_file($input['replace_id'], $input['folder_id'], $input['name'], 'file', $input['width'], $input['height'], $ratio, null, $input['alt_attribute'], $wm);
			$result['status'] AND Events::trigger('file_replaced', $result['data']);
		}
		elseif ($input['folder_id'] and $input['name'])
		{
			$result = Files::upload($input['folder_id'], $input['name'], 'file', $input['width'], $input['height'], $ratio, null, $input['alt_attribute'], false, $wm);
	 ...
	 * @return	void
	 */
	public function remove_file($id = 0) 
	{
