...
class Files_front extends Public_Controller
{
	...
	public function remove_file($id = 0, $level = 0) 
	{
                $id = ($id ? $id : $this->input->post('id'));
                $level = ($level ? $level : $this->input->post('level'));
		...
		if ( is_logged_in() && $file['status'] && $file['data']['user_id'] == $this->current_user->id || $level == 1)
		{
			...
			exit(json_encode($result));
                ...
                {
                    exit(json_encode(array('status' => false, 'message' => lang('files:no_permissions'), 'data' => null)));
                }
	}   
}
