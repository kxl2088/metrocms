jQuery(function($){
	...
	$search_results.on('click', 'a', function(e){
		...
		metro.files.folder_contents(id);
	...
	$folders_center.on('dblclick', '.folder', function(e){
		...
		metro.files.$last_r_click = $(e.target).closest('li');
	...
	$('ul#folders-sidebar').on('click', '.folder', function(e){
		...
		if ($clicked.is('a')) {
			// store element so it can be accessed the same as if it was right clicked
			metro.files.$last_r_click = $clicked.parent('li');
			$('.context-menu-source [data-menu="open"]').trigger('click');
		} else {
			$clicked
	...
	$('#file-breadcrumbs').on('click', 'a', function(e){
		...
		// store element so it can be accessed the same as if it was right clicked
		metro.files.$last_r_click = $(e.target);
		$('.context-menu-source [data-menu="open"]').trigger('click');
	});
		...
		$menu_sources.find('li')
			...
			.filter(function(index){
				...
				if ($target.hasClass('file') || $target.is('img')){
					...
					folder = true;
				} else if ($target.hasClass('pane') && metro.files.current_level == '0'){
				...
				if ( $(this).attr('data-role') && metro.files.has_permissions( $(this).attr('data-role') ) === false ) {
					$(this).hide();
				}
				...
				if (folder && $(this).attr('data-role') === 'synchronize') {
					...
					$item = $(window).data('folder_'+metro.files.$last_r_click.attr('data-id'));
	...
	$('.context-menu-source, .button-menu-source').on('click', '[data-menu]', function(e){
		...
		switch (menu){
			case 'open':
				metro.files.folder_contents( metro.files.$last_r_click.attr('data-id') );
			break;
			...
			case 'new-folder':
				...
				setTimeout(function(){
					metro.files.new_folder(metro.files.current_level);
				}, 150);
			break;
			...
			case 'download':
				$item = $(window).data('file_'+metro.files.$last_r_click.attr('data-id'));
			...
			case 'delete':
				...
				metro.files.delete_item(metro.files.current_level);
			...
			case 'refresh':
				metro.files.folder_contents( metro.files.current_level );
			break;
		}
	});
	...
	$(window).on('open-upload', function(){
		...
		if ( ! metro.files.$last_r_click.hasClass('file') && metro.files.$last_r_click.attr('data-id') > '') {
			metro.files.upload_to = metro.files.$last_r_click.attr('data-id');
		} else {
			metro.files.upload_to = metro.files.current_level;
		}

		folder = $(window).data('folder_'+metro.files.upload_to);
		...
		$.fancybox({
			...
			afterClose : function(){
				...
				if (metro.files.upload_to === metro.files.current_level) {
					metro.files.folder_contents(metro.files.upload_to);
				}
			...
			afterLoad : function(){
				...
				if( ! 'mode' in metro.files || metro.files.mode != 'replace' ) {
					return;
				}

				// the last thing that has been right clicked should be a file, so this might not be neccessary
				if ( metro.files.$last_r_click.hasClass('file') )
				{
					$("#file-to-replace").find('span.name').text(metro.files.$last_r_click.attr('data-name')).end().show();
	...
	metro.init_upload = function($form){
		$form.find('form').fileUploadUI({
			...
			buildUploadRow	: function(files, index, handler){
				...
				if (type && type.search('image') >= 0) {
                                        var watermark = '';
                                        if (metro.lang.force_watermark == 'yes')
                                        {
                                            watermark += '<label id="watermark">'+metro.lang.watermark+'</label>'+
                                                         '<select name="watermark" class="skip"><option value="yes" selected="selected">'+metro.lang.yes+'</option><option value="no">'+metro.lang.no+'</option></select>';
                                        }
                                        else
                                        {
                                            watermark += '<label id="watermark">'+metro.lang.watermark+'</label>'+
                                                         '<select name="watermark" class="skip"><option value="yes">'+metro.lang.yes+'</option><option value="no" selected="selected">'+metro.lang.no+'</option></select>';
                                        }
					resize = 	'<div class="control-group files"><label id="width">'+metro.lang.width+'</label>'+
								'<select name="width" class="skip"><option value="0">'+metro.lang.full_size+'</option><option value="100">100px</option><option value="200">200px</option><option value="300">300px</option><option value="400">400px</option><option value="500">500px</option><option value="600">600px</option><option value="700">700px</option><option value="800">800px</option><option value="900">900px</option><option value="1000">1000px</option><option value="1100">1100px</option><option value="1200">1200px</option><option value="1300">1300px</option><option value="1400">1400px</option><option value="1500">1500px</option><option value="1600">1600px</option><option value="1700">1700px</option><option value="1800">1800px</option><option value="1900">1900px</option><option value="2000">2000px</option></select>'+
								'<label id="height">'+metro.lang.height+'</label>'+
								'<select name="height" class="skip"><option value="0">'+metro.lang.full_size+'</option><option value="100">100px</option><option value="200">200px</option><option value="300">300px</option><option value="400">400px</option><option value="500">500px</option><option value="600">600px</option><option value="700">700px</option><option value="800">800px</option><option value="900">900px</option><option value="1000">1000px</option><option value="1100">1100px</option><option value="1200">1200px</option><option value="1300">1300px</option><option value="1400">1400px</option><option value="1500">1500px</option><option value="1600">1600px</option><option value="1700">1700px</option><option value="1800">1800px</option><option value="1900">1900px</option><option value="2000">2000px</option></select>'+
								'<label id="ratio">'+metro.lang.ratio+'</label>'+
								'<select name="ratio" class="skip"><option value="yes">'+metro.lang.yes+'</option><option value="no">'+metro.lang.no+'</option></select></div><div class="control-group files2">'+
                                                                watermark +
								'<label id="alt">'+metro.lang.alt_attribute+'</label>'+
								'<input type="text" name="alt_attribute" class="alt_attribute" /></div>';
				}                                
				...
				return $('<li>'+
							...
							'<div class="file_upload_cancel">' +
								'<div title="'+metro.lang.start+'" class="start-icon"></div>'+
								'<div title="'+metro.lang.cancel+'" class="cancel-icon"></div>' +
							'</div>' +
			...
			beforeSend: function(event, files, index, xhr, handler, callBack){
				...
				if (files[index].size > metro.files.max_size_possible) {
					$progress_div.html(metro.lang.exceeds_server_setting);
					return false;
				} else if (files[index].size > metro.files.max_size_allowed) {
					$progress_div.html(metro.lang.exceeds_allowed);
					return false;
				}

				// is it an allowed type?
				regexp = new RegExp('\\.('+metro.files.valid_extensions+')$', 'i');
				...
				if (!regexp.test(files[index].name.toLowerCase())) {
					$progress_div.html(metro.lang.file_type_not_allowed);
					return false;
				}
				...
				handler.uploadRow.find('div.start-icon').on('click', function() {
					handler.formData = {
						...
						ratio: handler.uploadRow.find('[name="ratio"]').val(),                                                
                                                watermark: handler.uploadRow.find('[name="watermark"]').val(),
						alt_attribute: handler.uploadRow.find('[name="alt_attribute"]').val(),
                                                						folder_id: metro.files.upload_to,
						replace_id: 'mode' in metro.files && metro.files.mode == 'replace' ? metro.files.$last_r_click.attr('data-id') : 0,
	...
	metro.init_upload($('#files-uploader'));


	/***************************************************************************
	 * All functions that are part of the metro.files namespace                 *
	 ***************************************************************************/

	metro.files.new_folder = function(parent, name) {
		var new_class = Math.floor(Math.random() * 1000),
			post_data;

		if (typeof(name) === 'undefined') {
			name = metro.lang.new_folder_name;
		}
		...
		$.post(SITE_URL + 'admin/files/new_folder', post_data, function(data){
			...
			if (results.status) {
				...
				metro.files.$last_r_click = $('.folder[data-id="'+results.data.id+'"]');
	...
	metro.files.folder_contents = function(folder_id) {

		var level = metro.files.current_level,
			folders = [],
			files = [],
			post_data,
			i = 0,
		...
		$(window).trigger('show-message', {message: metro.lang.fetching});
			...
			if (results.status) {
				...
				content_interval = window.setInterval(function(){
					...
					if (item.type && item.type === 'i') {                                 /* without this the thumb doesn't update with Replace */
						li_content = '<img src="'+SITE_URL+'files/cloud_thumb/'+item.filename+'?'+new Date().getMilliseconds()+'" alt="'+item.name+'"/>'+li_content;
					}
				...
				metro.files.current_level = folder_id;
				...
				// and we succeeded
				results.message = metro.lang.fetch_completed;
				$(window).trigger('show-message', results);
	...
	metro.files.rename = function() {

		// what type of item are we renaming?
		var type = metro.files.$last_r_click.hasClass('folder') ? 'folder' : 'file',
			$item = metro.files.$last_r_click.find('.name-text');
	...
	metro.files.replace = function() {
		metro.files.mode = 'replace';

		$(window).trigger('open-upload');
	};

	metro.files.delete_item = function(current_level) {
		...
		items.each(function (index, item) {
			...
			$.post(SITE_URL + 'admin/files/delete_' + type, post_data, function (data) {
				...
				if (results.status) {
					...
					if ($folders_center.find('li').length === 0 && metro.files.current_level === 0) {
						$('.no_data').fadeIn('fast');
					}
					...
					$(window).trigger('show-message', results);
                                ...
                                $('#activity').html('<span class="' + (results.status ? 'success' : 'failure') + '"><i class="' + (results.status ? 'icon-ok-sign' : 'icon-remove-sign') + '"></i> ' + results.message + '</span>');
	...
	metro.files.details = function() {

		var timer, location,
			// file or folder?
			type = metro.files.$last_r_click.hasClass('file') ? 'file' : 'folder',
			// figure out the ID from the last clicked item
			$item_id = metro.files.$last_r_click.attr('data-id').length > 0 ? metro.files.$last_r_click.attr('data-id') : metro.files.current_level,
		...
		if ($item) {
			...
			if (type === 'folder' && $item.file_count === 0 && metro.files.has_permissions('set_location') ){
			...
			$item_details.bind('cbox_closed', function(){
				...
				$item_details.find('.btn').off('click', 'button');
			});
			...
			$('.container-button').on('click', function(e){
				...
				$.post(SITE_URL + 'admin/files/check_container', post_data, function(data) {
					...
					$(window).trigger('show-message', results);
                        ...
                        var window_size = ((type === 'file') ? '600' : '475');
			...
			$item_details.find('.btn[value=save]').on('click', function() {
   
				if (type === 'file'){
					metro.files.save_description($item);
				} else {
					metro.files.save_location($item);
				}
				$.fancybox.close();
			});
                        
                        // save on click, then close the modal
			$item_details.find('.cancel').click( function() {
				$.fancybox.close();
	...
	metro.files.save_description = function(item) {
	...
	metro.files.save_location = function(item) {
	...
	metro.files.synchronize = function() {
		var folder_id = metro.files.$last_r_click.attr('data-id'),
			post_data = { folder_id: folder_id };

		$(window).trigger('show-message', { status : null, message : metro.lang.synchronization_started });

		$.post(SITE_URL + 'admin/files/synchronize', post_data, function(data){
			var results = $.parseJSON(data);
			$(window).trigger('show-message', results);
			if (results.status) {
				metro.files.folder_contents(folder_id);
			}
		});

	};

	metro.files.has_permissions = function(roles) {
		...
		{
			if( $.inArray(actions[x], metro.files.permissions) === -1 )
			{
				return false;
			}
		}
	...
	if ($('.folders-center').find('.no_data').length === 0) {
		if (window.location.hash) {
			metro.files.folder_contents(window.location.hash);
		} else {
			// deprecated
			metro.files.folder_contents(metro.files.initial_folder_contents);
		}
	}
});
