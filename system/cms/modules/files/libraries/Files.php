...
/**
 * MetroCMS File library. 
 *
 * This handles all file manipulation 
 * both locally and in the cloud
 * 
 * @author		Jerel Unruh - MetroCMS Dev Team
 * @package		MetroCMS\Core\Modules\Files\Libraries
 */
class Files
{
	...
	**/
	 ...
	 * @return array|bool
	 */
	public static function upload($folder_id, $name = false, $field = 'userfile', $width = false, $height = false, $ratio = false, $allowed_types = false, $alt = NULL, $replace_file = false, $watermark = false)
	{
		...
		if ($folder)
		{
			...
			if (ci()->upload->do_upload($field))
			{
				...
				if ($file['is_image'] and $watermark == true)
				{
					ci()->load->library('image_lib');
                                        $wm_config['image_library']        = 'gd2';       
                                        $wm_config['source_image']         = self::$path.$data['filename'];
                                        $wm_config['wm_type']              = Settings::get('files_wm_type');
                                        $wm_config['wm_vrt_alignment']     = Settings::get('files_wm_vrt_alignment');
                                        $wm_config['wm_hor_alignment']     = Settings::get('files_wm_hor_alignment');
                                        $wm_config['wm_vrt_offset']        = Settings::get('files_wm_vrt_offset') ? Settings::get('files_wm_vrt_offset') : 0;
                                        $wm_config['wm_hor_offset']        = Settings::get('files_wm_hor_offset') ? Settings::get('files_wm_hor_offset') : 0;
                                        $wm_config['wm_padding']           = Settings::get('files_wm_padding') ? Settings::get('files_wm_padding') : 0;
                                        
                                        //Config if watermark type is text
                                        $wm_config['wm_text']              = Settings::get('files_wm_text');
                                        $wm_config['wm_font_path']         = Settings::get('files_wm_font_path');
                                        $wm_config['wm_font_size']         = Settings::get('files_wm_font_size');
                                        $wm_config['wm_font_color']        = Settings::get('files_wm_font_color');
                                        $wm_config['wm_shadow_color']      = Settings::get('files_wm_shadow_color');
                                        $wm_config['wm_shadow_distance']   = Settings::get('files_wm_shadow_distance');
                                        
                                        //Check if the font file exist or try get file real path with files helper
                                        if( !file_exists($wm_config['wm_font_path']) )
                                        {
                                            if( file_real_path($wm_config['wm_font_path']) )
                                            {
                                                $wm_config['wm_font_path']     = file_real_path($wm_config['wm_font_path']);
                                            }
                                            else
                                            {
                                                $wm_config['wm_font_path']     = 'system/codeigniter/fonts/texb.ttf';
                                            }                                            
                                        }
                                         
                                        if( $wm_config['wm_type'] == 'text' and $wm_config['wm_text'] and $wm_config['wm_font_path'] and $wm_config['wm_font_size'] and  $wm_config['wm_font_color'] and $wm_config['wm_shadow_color'] and $wm_config['wm_shadow_distance'] )
                                        {
                                        ...
                                        $wm_config['wm_overlay_path']      = Settings::get('files_wm_overlay_path');
                                        $wm_config['wm_opacity']           = Settings::get('files_wm_opacity');
                                        $wm_config['wm_x_transp']          = Settings::get('files_wm_x_transp');
                                        $wm_config['wm_y_transp']          = Settings::get('files_wm_y_transp');
                                        
                                        //Check if the image file exist or try get file real path with files helper
                                        if( !file_exists($wm_config['wm_overlay_path']) )
                                        {
                                            if( file_real_path($wm_config['wm_overlay_path']) )
                                            {
                                                $wm_config['wm_overlay_path']     = file_real_path($wm_config['wm_overlay_path']);
                                            }
                                            else
                                            {
                                                unset($wm_config['wm_overlay_path']);
                                            }                                            
                                        }    
                                        
                                        if( $wm_config['wm_type'] == 'overlay' and $wm_config['wm_overlay_path'] and $wm_config['wm_opacity'] )
                                        {
                                                unset($wm_config['wm_font_path']);
                                                unset($wm_config['wm_font_size']);
                                                unset($wm_config['wm_font_color'] );
                                                unset($wm_config['wm_shadow_color']);
                                                unset($wm_config['wm_shadow_distance']);
	...
	**/
	 ...
	 *
	**/
	public static function replace_file($to_replace, $folder_id, $name = false, $field = 'userfile', $width = false, $height = false, $ratio = false, $allowed_types = false, $alt_attribute = NULL, $watermark = false)
	{
			...
			->get_by('files.id', $to_replace))
		{
			...
			$result = self::upload($folder_id, $name, $field, $width, $height, $ratio, $allowed_types, $alt_attribute, $file_to_replace, $watermark);
