(function($) {
	...
	$(function() {
		...
		metro.generate_slug('input[name="title"]', 'input[name="abbrev"]');

		var open_sections = $.cookie('nav_groups');		
                
		if (open_sections) {
			$('div[rel="'+open_sections+'"] .items').slideDown(200).removeClass('collapsed');
		} else {
			// show the first box with js to get around page jump
			$('.box .items:first').slideDown(200).removeClass('collapsed');			
		}
		
		// show and hide the sections
		$('.box .titles').click(function(){    
			window.scrollTo(0, 0);
			if ($(this).next('.items').hasClass('collapsed')) {
				$('.box .items').slideUp(200).addClass('collapsed');
				$.cookie('nav_groups', $(this).parents('.box').attr('rel'), { expires: 1, path: window.location.pathname });
				$(this).next('div.collapsed').slideDown(200).removeClass('collapsed');
			}
		});
		...
		$('#link-list li a').livequery('click', function()
		{
			...
			$('div.box .titles h3.group-title-'+id).html('');
		...
		$('.box:visible ul.sortable').livequery(function(){
			...
			// Get sortified
			metro.sort_tree($item_list, $url, $cookie, $data_callback);
		});
