 ...
 * @package    MetroCMS\Core\Modules\Navigation\Plugins
 */
class Plugin_Navigation extends Plugin
{
	...
	public function links()
	{
		...
		$links = $this->metrocache->model('navigation_m', 'get_link_tree', $params, config_item('navigation_cache'));
	...
	private function _build_links($links = array(), $return_arr = true)
	{
		...
		foreach ($links as $link)
		{
			...
			$item['url']   = $link['url'];
                        $item['title'] = ($this->parser->parse_string($link['lang'], false, true) ? $this->parser->parse_string($link['lang'], false, true) : $link['title']);  
			$item['total'] = $total;
