 ...
 * @package		MetroCMS\Core\Modules\Pages\Controllers
 */
class Pages extends Public_Controller
{
	...
	public function _page($url_segments)
	{
		...
		// results immediately.
		if (ENVIRONMENT == METRO_DEVELOPMENT)
		{
			$this->metrocache->delete_all('page_m');
		}

		// GET THE PAGE ALREADY. In the event of this being the home page $url_segments will be null
		$page = $this->metrocache->model('page_m', 'get_by_uri', array($url_segments, true));
		...
		if ( ! $page or ($page->status == 'draft' and ! $this->permission_m->has_role(array('put_live', 'edit_live'))))
		{
			...
			if ( ! ($page = $this->metrocache->model('page_m', 'get_by_uri', array('404'))) )
			{
		...
		if ($url_segments = explode('/', $page->base_uri) and count($url_segments) > 1)
		{
			...
			if ( ! $parents = $this->metrocache->get('page_m/'.md5(implode('/', $url_segments))))
			{
				...
				foreach ($url_segments as $segment)
				{
					...
					$parents[] = $this->metrocache->model('page_m', 'get_by_uri', array($breadcrumb_segments, true));
				}

				// Cache for next time
				$this->metrocache->write($parents, 'page_m/'.md5(implode('/', $url_segments)));
			}
                        
			foreach ($parents as $parent_page)
			{
				$this->template->set_breadcrumb(($this->parser->parse_string($parent_page->meta_title, false, true) ? $this->parser->parse_string($parent_page->meta_title, false, true) : $parent_page->title), $parent_page->uri);
			}
		}
		...
		$this->template->title($this->parser->parse_string($meta_title, $page, true))
			...
			->set_breadcrumb(($this->parser->parse_string($meta_title, $page, true) ? $this->parser->parse_string($meta_title, $page, true) : $page->title));
	...
	public function _rss($url_segments)
	{
		...
		// Hit the query through MetroCache.
		$children = $this->metrocache->model('page_m', 'get_many_by', array($query_options));
