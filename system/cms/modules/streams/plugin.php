...
class Plugin_Streams extends Plugin
{
	...
	public $description = array(
		'en' => 'A basic plugin for working with stream module.',
	);
	/**
	 * Field Types
	 *
	...
	public function __construct()
	{
		...
		// Load our MetroStreams-exclusive config
	...
	/**
	 * MetroStreams attribute function
	 *
	...
	private function tag_cache_write($content)
	{
		...
		{
			$this->metrocache->write($content, 'metrostreams'.DIRECTORY_SEPARATOR.$this->cache_hash, $this->cache);
		}		
	}
	...
	private function full_tag_cache()
	{
		...
		{
			if ( ! $tag_cache_content = $this->metrocache->get('metrostreams'.DIRECTORY_SEPARATOR.$this->cache_hash))
			{
	...
	public function total()
	{
		...
		{
			if ( ! $cache_content = $this->metrocache->get('metrostreams'.DIRECTORY_SEPARATOR.$this->cache_hash))
			{
				return $this->metrocache->write(
					$this->db->count_all(STR_PRE.$this->streams_attribute('stream')),
					'metrostreams'.DIRECTORY_SEPARATOR.$this->cache_hash, $this->cache);
	...
	public function calendar()
	{
		...
		foreach ($streams as $stream_slug)
		{
			...
			{
				$rows = $this->metrocache->model('row_m', 'get_rows', array($params, $this->fields, $stream), $this->cache);
			}
			else
			{
				$rows = $this->row_m->get_rows($params, $this->fields, $stream);
			...
			foreach ($rows as $above)
			{
				foreach ($above as $entry)
				{
					if (isset($displays[$count]))
					{
						...
						// Adding to the array
						if (isset($calendar[$entry['metrostreams_cal_day']]))
						{
							$calendar[$entry['metrostreams_cal_day']] .= $display_content.'<br />';
						}
						else
						{
							$calendar[$entry['metrostreams_cal_day']]  = $display_content.'<br />';
