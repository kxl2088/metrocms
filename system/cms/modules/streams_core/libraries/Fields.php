...
/**
 * MetroStreams Core Fields Library
 *
...
class Fields
{
     ...
     */
 	public function build_form($stream, $method, $row = false, $plugin = false, $recaptcha = false, $skips = array(), $extra = array(), $defaults = array())
 	{
		...
		$this->CI->load->language('streams_core/metrostreams');
		...
		if ($_POST and $key_check)
		{
			if ($this->CI->form_validation->run() === true)
			{
				...
				if ($extra['return'] or $plugin === true)
				{
                                    if($method == 'edit')
                                    {
					$_POST['btnAction'] == 'save_exit' ? redirect(str_replace('-id-', $result_id, $extra['return'])) : redirect(current_url());
                                    }
                                    else
                                    {
