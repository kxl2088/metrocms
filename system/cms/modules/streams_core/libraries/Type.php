...
/**
 * MetroStreams Core Field Type Library
 *
 * @package		MetroCMS\Core\Modules\Streams Core\Libraries
 * @author		Parse19
...
class Type
{
    ...
    {    
		...
		$this->CI->load->model('addons/module_m');
		...
		(CMS_VERSION < 1.0) ? define('METROSTREAMS_DB_PRE', '') : define('METROSTREAMS_DB_PRE', SITE_REF.'_');
		
		// Since this is MetroStreams core we know where
		// MetroStreams is, but we set this for backwards
		...
		// install situation. 
		if(defined('METROPATH'))
		{
			define('METROSTEAMS_DIR', METROPATH.'modules/streams_core/');
		}
		else
		{
			define('METROSTEAMS_DIR', APPPATH.'modules/streams_core/');
		}

		// Set our addon paths
		$this->addon_paths = array(
			'core' 			=> METROSTEAMS_DIR.'field_types/',
			'addon' 		=> ADDONPATH.'field_types/',
			'addon_alt' 	=> SHARED_ADDONPATH.'field_types/'
		);
                $modules = $this->CI->module_m->get_all();
                
                foreach ($modules as $module) {
                    
                    $path = $module['path'] . '/' . 'field_types/';
                    
                    if( is_dir($path) ){
                        $this->addon_paths[$module['slug']] = $path;
                    }
                }
