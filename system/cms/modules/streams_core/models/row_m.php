...
/**
 * MetroStreams Row Model
 *
 * @package		MetroCMS\Core\Modules\Streams Core\Models
 * @author		Parse19
...
class Row_m extends MY_Model {
	...
	public function get_rows($params, $field = null, $stream)
	{
		...
		{
			$this->sql['select'][] = 'DAY('.$this->format_mysql_date($date_by, $stream->stream_namespace).') as metrostreams_cal_day';
		}
	...
	{		
		$obj = $this->db->query('
			SELECT '.METROSTREAMS_DB_PRE.STREAMS_TABLE.'.*, '.METROSTREAMS_DB_PRE.STREAMS_TABLE.'.id as stream_id, '.METROSTREAMS_DB_PRE.FIELDS_TABLE.'.* 
			FROM '.METROSTREAMS_DB_PRE.STREAMS_TABLE.', '.METROSTREAMS_DB_PRE.ASSIGN_TABLE.', '.METROSTREAMS_DB_PRE.FIELDS_TABLE.'
			WHERE '.METROSTREAMS_DB_PRE.STREAMS_TABLE.'.id='.METROSTREAMS_DB_PRE.ASSIGN_TABLE.'.stream_id and
			'.METROSTREAMS_DB_PRE.FIELDS_TABLE.'.id='.METROSTREAMS_DB_PRE.ASSIGN_TABLE.'.field_id');
	...
	public function build_pagination($config, $limit, $total_rows, $pagination_vars, $pag_base = null)
	{
		...
		// -------------------------------------
                ...
                $pagination = array();
                $theme = $this->template->get_theme_path() . 'config/pagination.php';  
                if (is_file($theme)) require_once($theme);   
                
		$this->pagination->initialize($pagination_config+$pagination);
