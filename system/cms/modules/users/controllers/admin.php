...
class Admin extends Admin_Controller
{
	...
	public function create()
	{
		...
		$this->template
			...
			->set('member', $member)
                        ->append_js(array('module::form.js', 'module::form_create.js'))
        ...
        public function email_ajax_check()
	{
		...
		if ($this->ion_auth->email_check($this->input->post('email')))
		{
			echo json_encode(array('status' => 1, 'message' => lang('user_error_email')));
		}
                else
                {
		...
		echo json_encode(array('status' => 0, 'message' => ''));
            ...
            {
		echo json_encode(array('status' => 0, 'messagem' => ''));
            }
	}
        ...
        {
                header('Content-type: application/json');
                echo json_encode(array('password' => password(9, true, true, true)));
        }
        
        public function username_generator()
        {
                $username = '';
                $message = '';
                ...
                if ($this->input->post('first_name') and $this->input->post('last_name'))
                {
                        ...
                        $username = substr(url_title($this->input->post('first_name').'.'.$this->input->post('last_name'), '-', true), 0, 20);
                }
                else
                {
                        ...
                        $username = substr($email_parts[0], 0, 20);
                }
                ...
                $username_base = substr($username, 0, 18);
                ...
                {
                    $username = '';
                    $message = lang('user_generate_username_error');
                }
                
                header('Content-type: application/json');
                echo json_encode(array('username' => $username, 'message' => $message));
        }
