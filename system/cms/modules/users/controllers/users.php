 ...
 * @package		MetroCMS\Core\Modules\Users\Controllers
 */
class Users extends Public_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->library('form_validation');
                ...
                Asset::add_path('admin', $this->theme_m->get_admin()->web_path.'/');
	}
	...
	public function view($username = null)
	{
		...
		$this->template->title(sprintf_lang('profile_of_title', array($user->display_name)))
                        ->set_breadcrumb(sprintf_lang('profile_of_title', array($user->display_name)), '')
	...
	public function login()
	{
		...
		$this->template->title(lang('user:login_header'))
	...
	public function register()
	{
		...
		$validation = array(
			array(
				...
				'rules' => 'required|min_length['.$this->config->item('min_password_length', 'ion_auth').']|max_length['.$this->config->item('max_password_length', 'ion_auth').']'
                        ...
                        array(
				'field' => 'password2',
				'label' => lang('user:password_confirm_label'),
				'rules' => 'required|matches[password]|min_length['.$this->config->item('min_password_length', 'ion_auth').']|max_length['.$this->config->item('max_password_length', 'ion_auth').']'
			),
			array(
				'field' => 'email',
				'label' => lang('global:email'),
			...
			array(
				...
				'label' => lang('user_security_key'),
				'rules' => Settings::get('enable_recaptcha') == FALSE ? '' : 'required|callback__check_captcha_return',
			),
		);
		...
		$this->template
			->title(lang('user:register_title'))
                        ->set_breadcrumb(lang('user:register_title'), '')
                        ->append_js('module::jquery.maskedinput.min.js')
			->set('_user', $user)
			->build('register');
	}
	...
	public function activate($id = 0, $code = null)
	{
		...
		$this->template
			...
			->set_breadcrumb(lang('user:activate_account_title'), '')
			->build('activate');
	}
	...
	public function activated()
	{
		...
		$this->template
                        ->set_breadcrumb(lang('user:activated_account_title'), '')
	...
	{
                $this->template->set_breadcrumb(lang('user:reset_password_title'), '')
                               ->title(lang('user:reset_password_title'));
	...
	{
		METRO_DEMO and show_error(lang('global:demo_restrictions'));
		...
		$this->template
                        ->set_breadcrumb(lang('user:password_reset_title'), '')
	...
	public function edit($id = 0)
	{
		...
		// Render the view
		$this->template
                        ->title(lang('profile_edit'))
                        ->set_breadcrumb(lang('profile_edit'), '')
                        ->append_metadata("\n\t<script type=\"text/javascript\" src=\"". Asset::get_filepath_js('admin::jquery/jquery.maskedinput.min.js', true) ."\"></script>\n\t<script type=\"text/javascript\" src=\"". site_url('users/jquery/jquery.masks.config.js') ."\"></script>")
                        ->build('profile/edit', array(
			'_user' => $user,
	...
	public function _email_check($email)
	{
        ...
        public function _check_captcha_return()
        {
            $resp = new stdClass();
            ...
            if(!$this->input->post("recaptcha_challenge_field") OR !$this->input->post("recaptcha_response_field"))
            {
                
                $this->form_validation->set_message('_check_captcha_return', lang('user_required_security_key'));
                return false;
            ...
            else if (!$resp->is_valid) 
            {
                ...
                $this->form_validation->set_message('_check_captcha_return', lang('user_wrong_security_key'));
                return false;
        ...
        public function masked_settings()
	{
		if ( Settings::get('users_maskedinputplugin_code') )
		{
                    header('Content-type: text/javascript');
                    echo Settings::get('users_maskedinputplugin_code');
		}
                else
                {
                    header('Content-type: text/javascript');
