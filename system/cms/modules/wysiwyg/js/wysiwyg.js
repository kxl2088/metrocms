...
function insertImage(file, alt, location, path)
{
	...
	window.parent.instance.insertHtml('<img class="metro-image alignment-'+img_float+'" '+width_tag+' '+float_tag+' src="'+path+'" alt="' + alt + '" />');
    windowClose();
}
...
function insertFile(id, title, location, path)
{
	...
	window.parent.instance.insertHtml('<a class="metro-file" href="'+path+'">' + title + '</a>');
    windowClose();
}
...
$(function()
{
	function detectFile()
	{
		...
		if( ! element.hasClass('metro-image')) return false;
     ...
     */
    $('#files-nav li a').click( function(e) {
