...
class Plugin_Theme extends Plugin
{
	...
	public function _self_doc()
	{
		$info = array(
			'path' => array(// the name of the method you are documenting
				...
				'attributes' => array(
                                    'parse' => array(// this is the order-dir="asc" attribute
						'type' => 'bool',// Can be: slug, number, flag, text, array, any.
						'flags' => 'true|false',// flags are predefined values like this.
						'default' => 'true',// attribute defaults to this if no value is given
				...
				'attributes' => array(
					'name' => array(// this is the order-dir="asc" attribute
						...
						'required' => true,// is this attribute required?
                                        ...
                                        'module' => array(// this is the order-dir="asc" attribute
						'type' => 'text',// Can be: slug, number, flag, text, array, any.
						'flags' => '',// flags are predefined values like this.
	...
	public function partial()
	{
		...
		$string = $this->load->file($path . ( !isset($attributes['module']) ? 'partials/' : 'modules/' . $attributes['module'] . '/') . $name . ( !isset($attributes['module']) ? '.html' : '.php'), true);
	...
	public function path()
	{
		$path = & rtrim($this->load->get_var('template_views'), '/');
                $parse  = $this->attribute('parse', FALSE);
                
		return ($parse ? base_url() . preg_replace('#(\/views(\/web|\/mobile)?)$#', '', $path) . '/' : preg_replace('#(\/views(\/web|\/mobile)?)$#', '', $path) . '/');
	}
	...
	public function css()
	{
		...
		$rel   = $this->attribute('rel', 'stylesheet');
                ...
                foreach (array('file' => $file, 'title' => $title, 'media' => $media, 'type' => $type, 'rel' => $rel) as $key => $value) 
                {
                    if($key == 'file' && $value == "")
                    {
                        return '';
                ...
                $attributes = (!is_array($attributes) ? (array)$attributes : $attributes) + array('href' => $this->css_url($file));
                
                return link_tag($attributes);
	}
	...
	{
		$file = $this->attribute('file');
                $charset = $this->attribute('charset', 'utf-8');
                $type = $this->attribute('type', 'text/javascript');                
                $attributes = $this->attributes();
                
                foreach (array('file' => $file, 'type' => $type, 'charset' => $charset) as $key => $value) 
                {
                    if($key == 'file' && $value == "")
                    {
                        return '';
                ...
                $attributes = (!is_array($attributes) ? (array)$attributes : $attributes) + array('src' => $this->js_url($file));
                
		return script_tag($attributes);
	}
	 ...
	 * Usage:
	 *
	 *     {{ theme:lang lang="theme" line="theme_title" [default="MetroCMS"] }}
	 *
	 * @return string.
	 */
	public function lang()
	{
		...
		$deft_lang = CI::$APP->config->item('language');
                $file = CI::$APP->template->get_theme_path() . 'language/' . $deft_lang . '/' . $lang_file . '_lang' . EXT;
