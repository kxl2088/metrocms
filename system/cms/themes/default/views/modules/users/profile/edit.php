...
<?php endif; ?>
            ...
            <?php echo form_submit('', lang('profile_save_btn'), 'class="btn btn-small btn-primary"') ?>
            <?php echo form_close() ?>
    </div>
</div>
<script>
    $(document).ready(function() {
        $('#profile').find('input:not([type=file],[type=radio],[type=checkbox],[type=submit],[type=reset],[type=button]), select:not([name=dob_month],[name=dob_day],[name=dob_year])').addClass('form-control').css({ 'width': '40%' });
        $('#profile').find('select[name=dob_month],select[name=dob_day],select[name=dob_year]').addClass('form-control').css({'width': '23%', 'display': 'inline-block'});
    });
</script>
    </div>
</div>