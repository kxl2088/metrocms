...
function metroimage_onclick(e)
{
    update_instance();
    // run when metro button is clicked]
    CKEDITOR.currentInstance.openDialog('metroimage_dialog');
}
...
CKEDITOR.plugins.add('metroimages',
{
    requires: ['iframedialog'],
    init : function(editor)
    {
        CKEDITOR.dialog.addIframe('metroimage_dialog', 'Imagem', SITE_URL + 'admin/wysiwyg/image',800,500,function(){}, {
			onLoad: function(){
				var id = '#'+this.parts.contents.getId();
				$('.cke_dialog_page_contents', id).css({height:'100%'});
			}
		});
        editor.addCommand('metroimages', {exec:metroimage_onclick});
        editor.ui.addButton('metroimages',{ label:'Enviar ou inserir imagens da biblioteca.', command:'metroimages', icon:this.path+'images/icon.png' });
		...
		editor.on('selectionChange', function(evt)
		{
			 ...
			 */
			var command = editor.getCommand('metroimages'),
				element = evt.data.path.lastElement.getAscendant('img', true);

			// If nothing or a valid document
			if ( ! element || (element.getName() == 'img' && element.hasClass('metro-image')))
			{
		...
		editor.on('dialogShow', function(e) {
			...
			if (image && image.hasAttribute('data-metroimage')) {
			...
			elements:
			{
				'img': function(element)
				{
					...
					if (unprotectedSrc.indexOf('\{\{') != -1) {
						element.attributes['data-metroimage'] = true;
					}
