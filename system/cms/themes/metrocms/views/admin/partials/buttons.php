<?php if (isset($buttons) && is_array($buttons)): ?>
	...
	<?php foreach ($buttons as $key => $button): ?>
		...
		<?php switch ($button) :
			...
			?>
				<button type="submit" name="btnAction" value="delete" class="<?php echo $btn_class; ?> btn-danger confirm">
			...
			case 'close':
			case 'preview':
				if($btn_class == 'btn') $btn_class .= ' btn-success';
				$uri = 'admin/' . $this->module_details['slug'];
				$active_section = $this->load->get_var('active_section');
			...
			case 'edit':
				...
				if($btn_class == 'btn') $btn_class .= ' btn-primary';
